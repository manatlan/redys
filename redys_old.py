...
# https://stackoverflow.com/questions/41238569/cannot-create-with-async-sub-process-for-each-socket-connection-with-python-3-5
...
async def Server(address="/tmp/redys.socket",authkey="redys"):
    ...
    with multiprocessing.connection.Listener(address, authkey=authkey) as listener:
        ...
        while 1:
            ...
            x=loop.run_in_executor(executor, listener.accept )
            conn = await x
            print("====%s====" % type(conn))
